...
def shave_spikes(poly, shave_radius_m):
    """
    Remove narrow outward 'peaks' by doing an in-then-out buffer.
    Any spike thinner than ~2*radius will disappear.
    Uses meters if pyproj is installed; otherwise falls back to degrees.
    """
    if shave_radius_m <= 0:
        return poly
    ...
    if t_fwd:
        p_m = _reproject_poly(poly, t_fwd)
        r = float(shave_radius_m)
        q = p_m.buffer(-r).buffer(+r)
        return make_valid(_reproject_poly(q, t_back))

    # degree fallback (approx)
    deg_r = shave_radius_m / 111320.0
    q = poly.buffer(-deg_r).buffer(+deg_r)
    return make_valid(q)

def round_peaks_buffer(poly, buffer_round_m):
    """
    Round broader convex 'peaks' by doing an out-then-in buffer.
    This softens sharp outer corners by ~buffer_round_m.
    Uses meters if pyproj is available; otherwise falls back to degrees.
    """
    if buffer_round_m <= 0:
        return poly
    ...
    if t_fwd:
        p_m = _reproject_poly(poly, t_fwd)
        r = float(buffer_round_m)
        q = p_m.buffer(+r).buffer(-r)
        return make_valid(_reproject_poly(q, t_back))

    # degree fallback (approximate)
    deg_r = buffer_round_m / 111320.0
    q = poly.buffer(+deg_r).buffer(-deg_r)
    return make_valid(q)
...
def main():
    ...
    ap.add_argument("--shave_radius_m", type=float, default=0.0)
    ap.add_argument("--buffer_round_m", type=float, default=0.0)
    ...
    with rasterio.open(args.image_georef) as ds:
        ...
        processed = []
        for p in polys:
            p0 = shave_spikes(p, args.shave_radius_m)
            p0 = round_peaks_buffer(p0, args.buffer_round_m)
            p1 = smooth_polygon(p0, args.smooth_radius_m, args.simplify_tolerance_m)
