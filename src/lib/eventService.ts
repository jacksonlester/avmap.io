...
export async function getServiceStateAtDate(serviceId: string, date: Date) {
  const { data, error } = await supabase.rpc("get_service_state_at_date", {
...
export async function getAllServicesAtDate(date: Date) {
  ...
  const activeServices = Array.from(serviceStates.values())
    ...
    .map((service) => {
      const { isActive, geometry_name, vehicle_types, ...serviceData } =
        service;
      return {
        ...serviceData,
...
// Get timeline ticks for a specific service
export async function getServiceTimelineTicks(serviceId: string) {
  const events = await getServiceEvents(serviceId);
  ...
  const ticks = events.map((event) => {
    ...
    switch (event.event_type) {
      ...
      case "service_updated":
        ...
        details = "Service area or details changed";
        break;
      case "service_ended":
        description = "Service Ended";
        details = "Service discontinued";
        break;
      case "fares_policy_changed":
        description = "Fare Policy Changed";
        details = `Fares changed to: ${
          event.event_data.new_fares || "unknown"
        }`;
        break;
      case "access_policy_changed":
        description = "Access Policy Changed";
        details = `Access changed to: ${
          event.event_data.new_access || "unknown"
        }`;
        break;
      case "vehicle_types_changed":
      case "vehicle_types_updated":
        description = "Vehicle Types Changed";
        details = `New vehicles: ${
          event.event_data.new_vehicle_types || event.event_data.vehicleTypes || "unknown"
        }`;
        break;
      case "geometry_updated":
      case "Service Area Change":
        description = "Service Area Updated";
        details = "Service area boundaries changed";
        break;
      case "platform_updated":
        description = "Platform Updated";
        details = `Platform changed to: ${
          event.event_data.new_platform || "unknown"
        }`;
        break;
      case "supervision_updated":
      case "Supervision Change":
        description = "Supervision Updated";
        details = `Supervision changed to: ${
          event.event_data.new_supervision || event.event_data.supervision || "unknown"
        }`;
        break;
      case "Access Change":
        description = "Access Policy Changed";
        details = `Access changed to: ${
          event.event_data.access || "unknown"
        }`;
        break;
      case "Fares Change":
        description = "Fare Policy Changed";
        details = `Fares changed to: ${
          event.event_data.fares || "unknown"
        }`;
        break;
      case "Vehicle Change":
        description = "Vehicle Types Changed";
        details = `New vehicles: ${
          event.event_data.vehicleTypes || "unknown"
        }`;
        break;
      default:
